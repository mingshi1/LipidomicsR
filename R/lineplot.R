...
total.abundance<- function(data){
  ...
  ldt=pivot_longer(spdt,cols = -c(lipid_type,carbon_number,unsaturation), names_to = "species", values_to = "abundance")
  lp_sum=ldt%>%group_by(lipid_type,species)%>%summarise(sum=sum(abundance))
  cb_sum=ldt%>%group_by(carbon_number,species)%>%summarise(sum=sum(abundance))
  sat_sum=ldt%>%group_by(unsaturation,species)%>%summarise(sum=sum(abundance))
  res=data.frame()
  for (l in 1:3){
    if (l==1){
      dt=lp_sum
      name="total_abundance_lipid="
    }else if(l==2){
      dt=cb_sum
      name="total_abundance_carbon_number="
    }else{
      dt=sat_sum
      name="total_abundance_unsaturation="
    }
